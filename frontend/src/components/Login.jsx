...
export default function Login() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setAuth({ ...data.user, userId: data.user.id }, data.token);
    } catch (err) {
      setError(err.response?.data?.message || 'خطا در ورود/ثبت‌نام');
    }
  };
      ...
      <section className="relative grid w-full max-w-5xl overflow-hidden rounded-[2rem] bg-white shadow-2xl shadow-slate-950/40 ring-1 ring-white/10 lg:grid-cols-[1.1fr_0.9fr]">
        ...
        <div className="p-6 sm:p-10">
          ...
          <div className="mt-6 lg:mt-0">
            <p className="text-sm font-bold uppercase tracking-[0.24em] text-[#168acd]">Welcome</p>
            <h2 className="mt-3 text-3xl font-black tracking-tight text-slate-950 sm:text-4xl">
              {isRegister ? 'Create your account' : 'Sign in to your chat'}
            </h2>
            <p className="mt-3 text-sm leading-6 text-slate-500">
              {isRegister ? 'Start chatting with a clean modern interface.' : 'Continue your conversations from any screen.'}
            </p>
          </div>
          ...
          <button onClick={() => setIsRegister(!isRegister)} className="mt-6 w-full text-sm font-bold text-[#168acd] hover:underline">
